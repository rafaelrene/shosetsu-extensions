...
function defaults:latest(data)
	return self.parse(
		GETDocument(
			self.baseURL
				.. "/"
				.. self.novelListingURLPath
				.. "/page/"
				.. data[PAGE]
				.. "/?m_orderby=latest"
				.. self.novelListingURLSuffix
		)
	)
end
	...
	if loadChapters then
		if self.chaptersScriptLoaded then
			if self.ajaxUsesFormData then
				...
				doc = RequestDocument(
					POST(
						self.baseURL .. self.ajaxFormDataUrl,
						nil,
						FormBodyBuilder():add("action", "manga_get_chapters"):add("manga", id):build()
					)
				)
			else
				-- Used by BoxNovel, Foxaholic, NovelTrench, LightNovelHeaven, VipNovel and WoopRead.
				doc = RequestDocument(
					POST(self.baseURL .. "/" .. self.shrinkURLNovel .. "/" .. url .. self.ajaxSeriesUrl, nil, nil)
				)
			end
		end
...
return function(baseURL, _self)
	...
	if _self.searchHasOper then
		...
		filters[#filters + 1] =
			DropdownFilter(keyID, "Genres Condition", { "OR (any of selected)", "AND (all selected)" })
	end
